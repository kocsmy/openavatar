...
#if canImport(Security)
    /// Legacy (login) keychain, no entitlements required.
    ///
    /// We deliberately do NOT use kSecUseDataProtectionKeychain or a
    /// keychain-access-group entitlement. That combination needs a real Apple
    /// Developer team signature; on our ad-hoc (teamless) signature macOS's
    /// integrity daemon rejects the restricted entitlement and refuses to launch
    /// the app at all ("OpenAvatar.app can't be opened"). The legacy keychain
    /// launches and stores secrets reliably. The trade-off is the "wants to use
    /// your confidential information" prompt can reappear after an auto-update
    /// (the ACL is bound to the code signature, which changes each build) —
    /// annoying, but far better than an app that won't start. A permanent fix
    /// for the prompt requires Developer ID signing + notarization.
